...
#define APPLICATION_NAME        "Modified TCP Socket"
#define APPLICATION_VERSION     "0.1"

#define IP_ADDR             0x23A10648 /*EC2 IP: 35.161.6.72*/
#define PORT_NUM            5001
#define BUF_SIZE            1024
#define TCP_PACKET_COUNT    2
...
long UserInput()
{
    ...
    do{
        UART_PRINT("\r\n"
                   "Enter 1 to send file from LP1.");
        UART_PRINT("Enter: ");
        lRetVal = GetCmd(acCmdStore, sizeof(acCmdStore));
        if(lRetVal == 0){ //User pressed enter
          UART_PRINT("\n\n\rEnter Valid Input.");
        }
        else{
	  iInput  = (int)strtoul(acCmdStore,0,10);
          if(iInput  == 1){
	      // Wait to receive a '1' over UART
...
//****************************************************************************
//Can try running server and client on same launchpad
int BsdTcpServer(unsigned short usPort){
  return SUCCESS;
}
